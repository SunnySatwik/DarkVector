...
# ---------------------------------------------------------------------------
# 11. AI Semantic Calibration & Confidence Integrity (Sprint 15E.1)
# ---------------------------------------------------------------------------
def test_validator_rejects_confidence_reinterpretations():
    """
    Verifies that ResponseValidator catches and rejects invalid re-interpretations
    of confidence metrics, while allowing correct detection confidence labels.
    """
    policy = ResponsePolicy(
        route=PromptRoute.RISK_ANALYSIS,
        scope=ResponseScope.FOCUSED,
    ...
    # 1. Invalid re-interpretations: should fail
    bad_sentences = [
        "My analysis confidence is 95% for this alert.",
        "There is a 95% probability of compromise.",
        "There is 95% confidence that this is an attack.",
        "The analyst confidence is calibrated at 95%."
    ]

    for text in bad_sentences:
        full_text = f"{text}\n\nEvidence Used\n- Behavioral Detection Evidence"
        with pytest.raises(ValueError, match="Prohibited aggregate confidence reinterpretation"):
            ResponseValidator.validate_chat(full_text, "severity: HIGH", policy=policy)

    # 2. Legitimate phrases and context: should pass
    good_sentences = [
        "The underlying detection has 95% detection confidence.",
        "The powershell_encoded rule produced a HIGH-severity detection with 95% detection confidence.",
        "This is an investigation of a suspicious PowerShell pattern.",
        "We must analyze if an attacker executed malicious code."
    ]
    for text in good_sentences:
        full_text = f"{text}\n\nEvidence Used\n- Behavioral Detection Evidence"
        ResponseValidator.validate_chat(full_text, "severity: HIGH", policy=policy)


def test_grounding_instruction_has_semantic_calibration_clauses():
    """
    Checks that the strict grounding instructions generated by prompt builders
    contain strict guidelines distinguishing detection confidence from threat certainty.
    """
    from app.services.llm.behavioral_context import BehavioralReasoningContext
    from app.services.llm.prompt.builders.general import GeneralPromptBuilder

    beh_ctx = BehavioralReasoningContext.from_context({
        "behavioral_context": {
            "is_behavioral": True,
            "primary_detection": {"rule_id": "powershell_encoded"}
        }
    })
    builder = GeneralPromptBuilder("Knowledge Doc Text", behavioral_context=beh_ctx)
    prompt = builder.build()
    
    assert "Never invent, calculate, or output any 'analysis confidence'" in prompt
    assert "not by itself proof of malicious intent" in prompt
    assert "unsupported threat escalation" in prompt


def test_fallback_ai_preserves_confidence_and_threat_semantics():
    """
    Verifies that FallbackAI focused chat generation output conforms to confidence
    integrity and does not over-escalate to compromise certainty.
    """
    class MockInvestigation:
        severity = "HIGH"
        risk_score = 95.0
    ...
    # Assertions on fallback output guarantees
    assert "with 95% detection confidence" in reply
    assert "does not by itself establish malicious intent or host compromise" in reply
    assert "analysis confidence" not in reply.lower()
